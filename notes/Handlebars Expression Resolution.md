...
A bare identifier resolves to an assign at the top level and to the current item inside `{{#each}}`. Keys that are not valid identifiers — dashes, spaces, dots, leading digits, or reserved words — are written as **bracketed literal segments**: `{{[first-name]}}`, `{{user.[first-name]}}`, `{{[a.b]}}`; a numeric bracket segment is a list index (`{{items.[2]}}`). Bare names may also use any leading letter case (`{{Item1}}`). Stem supports nested subexpressions `(helper arg)` — which must wrap a call or pipeline and are valid only in argument position — and helper pipelines (`lhs | helper a b`), the pipe being the linear form of the same composition `(helper (other x))`. Anything else is rejected as a `Stem.SyntaxError` — there is no arbitrary-Elixir fallback.
