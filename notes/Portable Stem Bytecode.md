...
- Partial arguments (`{{> name ctx key=value}}`) lower to `{:scope, base, hash, body}`: `base` is the context `value_op` (or `{:this}` inside an each, else `{:assigns}`) and `hash` is the keyword pairs. The VM evaluates base+hash in the caller scope, builds the merged scope via `Stem.Runtime.partial_scope/2`, and renders `body` with the assigns rebound and the block-scoped state reset — matching the compiled backend's closure rebinding (see [[Helper and Partial Resolution]]). An argument-less partial still expands inline and emits no `:scope`.
