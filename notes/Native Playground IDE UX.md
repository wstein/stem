...
- The Plain Text and View Model views share one **read-only CodeMirror editor** rather than a hand-built `<pre>`. Its language auto-switches: YAML for View Model, and for Plain Text the example's output format (rendered→HTML, markdown→Markdown, plain `source`→none). A "Highlight" combobox (a searchable `<input list>` + `<datalist>`, shown only for Plain Text) overrides the autoswitch with any of the ~140 languages in CodeMirror's `@codemirror/language-data` registry — type to search, with case-insensitive and alias matching (e.g. `ts`→TypeScript); an unrecognized entry reverts to the last valid pick. Each language lazy-loads its grammar via the registry's `load()` on first use and is cached; only the lightweight metadata is imported up front. The `language-data` import is dep-pinned so the lazily-loaded grammars reuse this page's `@codemirror/state·view·language` instances (avoiding CodeMirror's duplicate-instance failure). AsciiDoc has no CodeMirror grammar, so it is absent from the registry. Reusing CodeMirror gives the output the same theme-aware highlighting as the source editors with no bundled highlighter dependency, and keeps the trust boundary intact — CodeMirror tokenizes text into its own escaped DOM, never `innerHTML` of raw output.
