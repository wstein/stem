...
- The output pane offers three views: "Rendered" (the output as text, segment-mapped to its source), "Preview" (the output HTML in a locked-down iframe), and "Markdown" (the output interpreted as Markdown in the same iframe via a vendored `marked`). The iframe sandbox (no scripts; CSP `default-src 'none'`) is the trust boundary for both iframe views.
