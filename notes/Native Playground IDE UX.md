...
- Source-map provenance: the compiler can emit per-instruction `src` spans and the renderer a segment map (see [[Native AST Compilation Pipeline]]). In the Rendered view, hovering an output run shows a tooltip naming its source file/tag and highlights the originating span in the template editor (or tints its tab when that file is not shown); clicking jumps the caret there. Hovering never moves the caret.
