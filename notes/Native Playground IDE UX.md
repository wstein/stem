...
- The Plain Text and View Model views share one **read-only CodeMirror editor** rather than a hand-built `<pre>`. Its language auto-switches: YAML for View Model, and for Plain Text the example's output format (rendered→HTML, markdown→Markdown, plain `source`→none). Reusing CodeMirror gives the output the same theme-aware highlighting as the source editors with no new highlighter dependency, and keeps the trust boundary intact — CodeMirror tokenizes text into its own escaped DOM, never `innerHTML` of raw output.
