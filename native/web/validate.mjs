...
for (const ex of manifest) {
  ...
  // Honor the example's declared capability groups (the playground does too);
  // some examples — e.g. the cheat-sheet — need `eval` to render at all.
  const transformers = ex.groups ? ["minimum", ...ex.groups] : undefined;
  const rendered = render(compiled.program, data, { transformers });
  if (want === undefined) {
    // No pinned expected output (e.g. the large, data-driven cheat-sheet):
    // smoke-test that it compiles and renders without erroring.
...
for (const ex of manifest) {
  ...
  const transformers = ex.groups ? ["minimum", ...ex.groups] : undefined;
  const { output, segments } = render(compiled.program, data, { map: true, transformers });
  if (output !== render(compiled.program, data, { transformers })) {
    mapFailures++;
